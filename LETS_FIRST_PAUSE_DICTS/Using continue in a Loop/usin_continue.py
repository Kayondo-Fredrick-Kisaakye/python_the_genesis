...
# at 8, so current_number is 1. The if statement then checks the modulo of 
# current_number and 2. If the modulo is 0 (which means current_number is divisible by 2),
# the continue statement tells Python to ignore the rest of 
# the loop and return to the beginning. If the current number is not divisible by 2,
# the rest of the loop is executed and Python prints the current number:
